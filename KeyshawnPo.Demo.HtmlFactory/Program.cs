...
namespace KeyshawnPo.Demo.HtmlFactory
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //根据模板生成文章
            string _artilce = VirtualArticle();
            Console.WriteLine(_artilce);
        ...
        public static string VirtualArticle()
        {
            ...
            _VirtualParams.ParamValue = "甲方公司名称";
            ...
            _VirtualParams.ParamValue = "乙方公司名称";
            _VirtualParams.ParamRemarks = "乙方公司名称";
            _VirtualParams.ParamVersion = "V1.0";
            _lstVirtualParams.Add(_VirtualParams);

            //信息模板
            ArticleTmpleteEntity _QiMArticle_20180612 = new ArticleTmpleteEntity();
            _QiMArticle_20180612.AriticleTemplte = new Dictionary<TempleteType, string>();
            _QiMArticle_20180612.AriticleTemplte.Add(TempleteType.title, " 标题模板，企业合作推广模板 \br" + " 年月日 ，#CompanyAName# 与 #CompanyBName# 合作 ！");
            _QiMArticle_20180612.AriticleTemplte.Add(TempleteType.content, "内容模板， 企业合作推广信息 \br" + " 年月日 ，#CompanyAName# 与 #CompanyBName# 合作 ！");


            ArticleEntity _Article = new ArticleEntity();

            //根据参数替换模板内容
            foreach (var _vTemple in _QiMArticle_20180612.AriticleTemplte)
            {
                string _aritcltParti = _vTemple.Value;

                foreach (var _vParams in _lstVirtualParams)
                {
                    _aritcltParti = _aritcltParti.Replace('#' + _vParams.ParamId + '#', _vParams.ParamValue);
                }

                switch (_vTemple.Key)
                {
                    case TempleteType.title:
                        _Article.Title = _aritcltParti;

                        break;
                    case TempleteType.content:
                        _Article.Content = _aritcltParti;

                        break;
                    case TempleteType.other:
                        break;
                    default:
                        break;
                }
            ...
            //生成文章预览
            _article = JsonConvert.SerializeObject(_Article);
