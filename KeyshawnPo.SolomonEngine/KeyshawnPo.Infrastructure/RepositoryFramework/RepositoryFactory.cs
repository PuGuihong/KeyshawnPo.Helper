            ...
            where TEntity : EntityBase
        {
            ...
            if (!RepositoryFactory.repositories.ContainsKey(interfaceShortName))
            {
                RepositorySettings settings =
                    (RepositorySettings)ConfigurationManager.
                    GetSection(RepositoryMappingConstants.
                    RepositoryMappingsConfigurationSectionName);

                repository = Activator.CreateInstance(
                    Type.GetType(settings.RepositoryMappings[interfaceShortName].RepositoryFullTypeName)
                    ) as TRepository;
