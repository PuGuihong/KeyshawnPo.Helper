...
namespace KeyshawnPo.Helper.Lib.Applay.WebHelper
{
    /// <summary>
    /// web帮助类
    /// </summary>
    public class WebHelper
    {
        ...
        public static string GetIP()
        {
            ...
            if (string.IsNullOrEmpty(ip) || !ValidatorHelper.IsIP(ip))
                ip = "127.0.0.1";
            return ip;
        }
        ...
        private static bool TryWriteWebConfig()
        {
            try
            {
                File.SetLastWriteTimeUtc(FileDownHelper.GetMapPathFile("~/web.config"), DateTime.UtcNow);
                return true;
            }
            catch
            {
        ...
        private static bool TryWriteGlobalAsax()
        {
            try
            {
                File.SetLastWriteTimeUtc(FileDownHelper.GetMapPathFile("~/global.asax"), DateTime.UtcNow);
                return true;
            }
            catch
            {
        ...
        /// <param name="ipAddress">结束IP</param>
        /// <returns>小于：-1  等于：0  大于：1  失败：-9</returns>
        public static int Compare(string startIP, string ipAddress)
        {
            int tmpRes = -9;
            try
            {
                byte[] start_ip_array = Get4Byte(startIP);//将点分十进制 转换成 4个元素的字节数组
                byte[] ip_array = Get4Byte(ipAddress);

                for (int i = 0; i < 4; i++)
                {
                    //从左到右 依次比较 对应位置的 值的大小  ，一旦检测到不在对应的范围 那么说明IP不在指定的范围内 并将终止循环
                    if (start_ip_array[i] == ip_array[i])
                    {
                        tmpRes = 0;
                    }
                    else if (start_ip_array[i] < ip_array[i])
                    {
                        tmpRes = 1; break;
                    }
                    else
